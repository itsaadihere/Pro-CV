    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 800, color: '#000000', letterSpacing: '0.1em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
...
export default function MIN01BlackWhiteMinimalistAccountant({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: s(18) }}>
        <div>
          <h1 style={{ fontSize: s(24), fontWeight: 900, color: '#000000', margin: 0, letterSpacing: '-0.5px' }}>{data.fullName}</h1>
          <div style={{ height: s(2), backgroundColor: '#000000', width: s(40), marginTop: s(4) }} />
        </div>
        <div style={{ textAlign: 'right', fontSize: s(9.5), color: '#4b5563', lineHeight: '1.4' }}>
          <strong style={{ display: 'block', color: '#000000', fontSize: s(10), textTransform: 'uppercase', marginBottom: s(2) }}>{data.jobTitle}</strong>
