    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#000000', textTransform: 'uppercase' }}>{label}</h2>
...
export default function MIN02BlackWhiteSimpleInfographic({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: s(20), borderBottom: `${s(1)} solid #e5e7eb`, paddingBottom: s(10) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 800, color: '#000000', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(9.5), color: '#4b5563', display: 'flex', gap: s(12) }}>
    ...
    <div style={{ marginBottom: s(16) }}>
      <h3 style={{ fontSize: s(10), fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isLeftSidebar ? '#ffffff' : '#000000', marginBottom: s(6) }}>Contact</h3>
    ...
    {Object.keys(data.technicalSkills).length > 0 && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: s(9.5), fontWeight: 600, color: isTwoColumn ? '#ffffff' : '#000000' }}>
              <span>{cat}</span>
              <div style={{ display: 'flex', gap: s(3) }}>
                {[1,2,3,4,5].map(dot => (
                  <div key={dot} style={{ width: s(5), height: s(5), borderRadius: '50%', backgroundColor: dot <= 4 ? '#000000' : 'rgba(255,255,255,0.2)' }} />
                ))}
              </div>
            </div>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <strong style={{ color: isLeftSidebar ? '#ffffff' : '#000000' }}>{edu.degree}</strong>
