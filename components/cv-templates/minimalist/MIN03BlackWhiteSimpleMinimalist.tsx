    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(18), textAlign: 'center' }}>
          <div style={{ height: s(0.5), backgroundColor: '#000000', margin: `${s(4)} auto` }} />
...
export default function MIN03BlackWhiteSimpleMinimalist({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: s(24) }}>
        <h1 style={{ fontSize: s(28), fontWeight: 800, color: '#000000', margin: 0, letterSpacing: '1px', textTransform: 'uppercase' }}>{data.fullName}</h1>
        <div style={{ fontSize: s(11), color: '#4b5563', marginTop: s(4), fontWeight: 500, letterSpacing: '2px', textTransform: 'uppercase' }}>{data.jobTitle}</div>
        <div style={{ height: s(1), backgroundColor: '#000000', margin: `${s(8)} auto`, width: '60%' }} />
