    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(12), fontWeight: 800, color: '#2563eb', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1), backgroundColor: '#2563eb', marginTop: s(4) }} />
        </div>
      );
    };
    

export default function MIN04BlueSimpleProfessional1({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20) }}>
        ...
        <div style={{ height: s(3), backgroundColor: '#2563eb', margin: `${s(6)} 0`, borderRadius: s(1.5) }} />
