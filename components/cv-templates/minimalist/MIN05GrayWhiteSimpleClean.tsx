      ...
      if (isSidebar) {
        return (
          <div style={{ marginBottom: s(10), marginTop: s(14) }}>
            <h2 style={{ fontSize: s(11), fontWeight: 700, color: '#111827', textTransform: 'uppercase' }}>{label}</h2>
          </div>
        );
      }
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14), borderLeft: `${s(3)} solid #9ca3af`, paddingLeft: s(6) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase' }}>{label}</h2>
        </div>
      );
    };
    

export default function MIN05GrayWhiteSimpleClean({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: '#e5e7eb', color: '#111827', padding: `${s(18)} ${s(24)}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: s(20), borderRadius: s(4) }}>
        <div>
