    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(10), fontWeight: 700, color: '#6b7280', letterSpacing: '0.15em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(0.5), backgroundColor: '#d1d5db', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function MIN07MinimalistWhiteGreyProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: s(18) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 800, color: '#111827', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(9.5), color: '#6b7280', display: 'flex', gap: s(12) }}>
          {data.email && <span>{data.email}</span>}
          {data.phone && <span>{data.phone}</span>}
        </div>
      </div>
      <div style={{ height: s(1), backgroundColor: '#d1d5db', marginBottom: s(16) }} />
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span style={{ fontSize: s(9), color: '#6b7280' }}>{job.startDate} – {job.endDate}</span>
              </div>
              <div style={{ fontSize: s(10), color: '#d1d5db', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
      ...
      <section style={{ marginBottom: s(16) }}>
        ...
        {data.certifications.map((cert, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), marginBottom: s(4), color: '#374151' }}>
            ...
            <span style={{ color: '#6b7280', fontSize: s(8.5) }}>{cert.year}</span>
          </div>
        ))}
      </section>
    )}
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: '#6b7280', fontSize: s(9) }}>{edu.startYear} - {edu.endYear}</span>
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ fontSize: s(9.5) }}>
            ...
            <span style={{ color: isTwoColumn ? '#d1d5db' : '#6b7280' }}>{skills.join(', ')}</span>
          </div>
        ))}
      </div>
