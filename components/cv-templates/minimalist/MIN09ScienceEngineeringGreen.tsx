    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 800, color: '#16a34a', letterSpacing: '0.08em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#16a34a', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function MIN09ScienceEngineeringGreen({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: '#000000', color: '#ffffff', padding: `${s(20)} ${s(24)}`, marginBottom: s(20), borderRadius: s(4), display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: s(22), fontWeight: 800, margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(11), color: '#16a34a', marginTop: s(2), fontWeight: 600 }}>{data.jobTitle}</div>
        </div>
        <div style={{ fontSize: s(10), textAlign: 'right', display: 'flex', flexDirection: 'column', gap: s(2), opacity: 0.9 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#16a34a', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#16a34a15', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #16a34a` }}>
            {skill}
          </span>
        ))}
      </div>
