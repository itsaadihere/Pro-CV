    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 800, color: '#000000', letterSpacing: '0.08em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#000000', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function MIN10ScienceEngineeringWhite({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 850, color: '#000000', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(11), color: '#4b5563', marginTop: s(2) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', gap: s(12), fontSize: s(9.5), color: '#4b5563', marginTop: s(6) }}>
