    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(18), textAlign: 'center' }}>
          <div style={{ height: s(0.5), backgroundColor: '#9ca3af', margin: `${s(4)} auto` }} />
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#000000', letterSpacing: '0.12em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(0.5), backgroundColor: '#9ca3af', margin: `${s(4)} auto` }} />
        </div>
      );
    };
...
export default function MIN11SimpleElegantCleanClassic({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: s(24) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 700, color: '#000000', margin: 0, fontFamily: 'Georgia, serif' }}>{data.fullName}</h1>
        <div style={{ height: s(1), backgroundColor: '#000000', margin: `${s(6)} auto`, width: s(60) }} />
        <div style={{ fontSize: s(11), fontStyle: 'italic', color: '#4b5563', marginBottom: s(6) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(16), fontSize: s(9.5), color: '#4b5563' }}>
