...
import { CVTemplateProps } from '@/types/cv'
    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(16), borderLeft: `${s(4)} solid #1e3b8a`, paddingLeft: s(8) }}>
          <h2 style={{ fontSize: s(12), fontWeight: 800, color: '#111827', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
        </div>
      );
    };
    

export default function MIN13WhiteModernBusinessAdmin({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: s(20), borderBottom: `${s(1)} solid #e5e7eb`, paddingBottom: s(10) }}>
        <div>
          <h1 style={{ fontSize: s(22), fontWeight: 800, color: '#111827', margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(11), color: '#1e3b8a', marginTop: s(2), fontWeight: 600 }}>{data.jobTitle}</div>
        </div>
        <div style={{ fontSize: s(9.5), color: '#4b5563', textAlign: 'right', display: 'flex', flexDirection: 'column', gap: s(2) }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#1e3b8a', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#1e3b8a15', borderRadius: s(4), color: '#111827', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #1e3b8a` }}>
            {skill}
          </span>
        ))}
      </div>
