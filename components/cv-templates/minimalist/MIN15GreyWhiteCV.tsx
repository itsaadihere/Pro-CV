    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 700, color: isSidebar ? '#ffffff' : '#ffffff', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1), backgroundColor: isSidebar ? 'rgba(255,255,255,0.2)' : '#e5e7eb', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function MIN15GreyWhiteCV({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), textAlign: 'center' }}>
        {false && <div style={{ width: s(80), height: s(80), borderRadius: '50%', backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto ' + s(12), overflow: 'hidden' }}>
          <span style={{ color: '#374151', fontSize: s(22), fontWeight: 800 }}>{data.fullName.substring(0, 2)}</span>
        </div>}
    ...
    <div style={{ marginBottom: s(16) }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(4), fontSize: s(9.5), color: isLeftSidebar ? '#d1d5db' : '#d1d5db' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ fontSize: s(9.5) }}>
            <strong style={{ color: isTwoColumn ? '#ffffff' : '#ffffff' }}>{cat}: </strong>
            <span style={{ color: isTwoColumn ? '#d1d5db' : '#d1d5db' }}>{skills.join(', ')}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            ...
            <div style={{ color: isLeftSidebar ? '#d1d5db' : '#d1d5db' }}>{edu.institution}</div>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span style={{ fontSize: s(9), color: '#d1d5db' }}>{job.startDate} – {job.endDate}</span>
              </div>
              <div style={{ fontSize: s(10), color: '#374151', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
      ...
      <section style={{ marginBottom: s(16) }}>
        ...
        {data.certifications.map((cert, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), marginBottom: s(4), color: '#374151' }}>
            ...
            <span style={{ color: '#d1d5db', fontSize: s(8.5) }}>{cert.year}</span>
          </div>
        ))}
      </section>
    )}
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: '#d1d5db', fontSize: s(9) }}>{edu.startYear} - {edu.endYear}</span>
            </div>
            <div style={{ color: '#d1d5db' }}>{edu.institution}</div>
          </div>
        ))}
      </section>
    )}
