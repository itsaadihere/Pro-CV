    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#06b6d4', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
        </div>
      );
    };
    

export default function MIN16WhiteSimpleStudent1({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
