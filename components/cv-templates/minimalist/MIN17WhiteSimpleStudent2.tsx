    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#7c3aed', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#7c3aed', marginTop: s(3), width: '100%', borderRadius: s(1) }} />
        </div>
      );
    };
    

export default function MIN17WhiteSimpleStudent2({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: s(20) }}>
        <div>
          <h1 style={{ fontSize: s(24), fontWeight: 850, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(11), color: '#7c3aed', fontWeight: 600, marginTop: s(2) }}>{data.jobTitle}</div>
        </div>
        <div style={{ width: s(36), height: s(36), backgroundColor: '#7c3aed', opacity: 0.1, borderRadius: s(8) }} />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#7c3aed', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
