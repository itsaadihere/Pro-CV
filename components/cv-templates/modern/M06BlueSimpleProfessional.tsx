    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#2563eb', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#2563eb', marginTop: s(3), width: '100%', borderRadius: s(1) }} />
        </div>
      );
    };
    

export default function M06BlueSimpleProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), textAlign: 'center' }}>
        <h1 style={{ fontSize: s(28), fontWeight: 800, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
        <div style={{ height: s(3), backgroundColor: '#2563eb', margin: `${s(8)} auto`, width: s(80), borderRadius: s(2) }} />
        <div style={{ fontSize: s(12), color: '#64748b', fontWeight: 500, marginBottom: s(6) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(16), fontSize: s(10), color: '#64748b' }}>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                <span style={{ fontSize: s(9), color: '#64748b' }}>{job.startDate} – {job.endDate}</span>
              </div>
      ...
      <section style={{ marginBottom: s(16) }}>
        ...
        {data.certifications.map((cert, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), marginBottom: s(4), color: '#374151' }}>
            ...
            <span style={{ color: '#64748b', fontSize: s(8.5) }}>{cert.year}</span>
          </div>
        ))}
      </section>
    )}
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: '#64748b', fontSize: s(9) }}>{edu.startYear} - {edu.endYear}</span>
            </div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#2563eb15', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #2563eb` }}>
            {skill}
          </span>
        ))}
      </div>
