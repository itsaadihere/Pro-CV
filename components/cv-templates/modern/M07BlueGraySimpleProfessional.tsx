    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: isSidebar ? '#1e293b' : '#2563eb', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
...
export default function M07BlueGraySimpleProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
