    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 700, color: isSidebar ? '#ffffff' : '#1e3a5f', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
...
export default function M08BlueWhiteModernProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), textAlign: 'center' }}>
        <div style={{ width: s(90), height: s(90), borderRadius: '50%', backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto ' + s(12), overflow: 'hidden' }}>
          <span style={{ color: '#1e3a5f', fontSize: s(24), fontWeight: 800 }}>{data.fullName.substring(0, 2)}</span>
        </div>
        <h1 style={{ fontSize: s(18), fontWeight: 800, color: '#ffffff', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(10), color: '#dbeafe', marginTop: s(4) }}>{data.jobTitle}</div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(8) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ marginBottom: s(2) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), fontWeight: 600, color: isTwoColumn ? '#ffffff' : '#1e3a5f', marginBottom: s(2) }}>
              <span>{cat}</span>
            </div>
            <div style={{ height: s(5), backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: s(2.5), overflow: 'hidden' }}>
              <div style={{ width: '85%', height: '100%', backgroundColor: '#1e3a5f' }} />
            </div>
