    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 700, color: isSidebar ? '#ffffff' : '#1e293b', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
...
export default function M10DeepPurpleCollegeStudent({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), textAlign: 'center' }}>
        {true && <div style={{ width: s(80), height: s(80), borderRadius: '50%', backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto ' + s(12), overflow: 'hidden' }}>
          <span style={{ color: '#4c1d95', fontSize: s(22), fontWeight: 800 }}>{data.fullName.substring(0, 2)}</span>
        </div>}
    ...
    <div style={{ marginBottom: s(16) }}>
      <h3 style={{ fontSize: s(10), fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isLeftSidebar ? '#ffffff' : '#1e293b', marginBottom: s(6) }}>Contact</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(8) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ marginBottom: s(2) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), fontWeight: 600, color: isTwoColumn ? '#ffffff' : '#1e293b', marginBottom: s(2) }}>
              <span>{cat}</span>
            </div>
            <div style={{ height: s(5), backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: s(2.5), overflow: 'hidden' }}>
              <div style={{ width: '85%', height: '100%', backgroundColor: '#4c1d95' }} />
            </div>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <strong style={{ color: isLeftSidebar ? '#ffffff' : '#1e293b' }}>{edu.degree}</strong>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#4c1d95', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
