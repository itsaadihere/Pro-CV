    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#14532d', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#14532d', marginTop: s(3), width: '100%', borderRadius: s(1) }} />
        </div>
      );
    };
    

export default function M11GreenElegantProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), textAlign: 'center' }}>
        {true && <div style={{ width: s(80), height: s(80), borderRadius: '50%', backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto ' + s(12), overflow: 'hidden' }}>
          <span style={{ color: '#14532d', fontSize: s(22), fontWeight: 800 }}>{data.fullName.substring(0, 2)}</span>
        </div>}
        <h1 style={{ fontSize: s(18), fontWeight: 800, color: '#ffffff', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(10), opacity: 0.8, marginTop: s(4) }}>{data.jobTitle}</div>
      </div>
    
            
    {/* Sidebar contact info */}
    <div style={{ marginBottom: s(16) }}>
      <h3 style={{ fontSize: s(10), fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isLeftSidebar ? '#ffffff' : '#14532d', marginBottom: s(6) }}>Contact</h3>
    ...
    {Object.keys(data.technicalSkills).length > 0 && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: s(9.5), fontWeight: 600, color: isTwoColumn ? '#ffffff' : '#14532d' }}>
              <span>{cat}</span>
              <div style={{ display: 'flex', gap: s(3) }}>
                {[1,2,3,4,5].map(dot => (
                  <div key={dot} style={{ width: s(5), height: s(5), borderRadius: '50%', backgroundColor: dot <= 4 ? '#14532d' : 'rgba(255,255,255,0.2)' }} />
                ))}
              </div>
            </div>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <strong style={{ color: isLeftSidebar ? '#ffffff' : '#14532d' }}>{edu.degree}</strong>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <strong style={{ fontSize: s(11), color: '#14532d' }}>{job.title}</strong>
                <span style={{ fontSize: s(9), color: '#4b5563' }}>{job.startDate} – {job.endDate}</span>
              </div>
              <div style={{ fontSize: s(10), color: '#14532d', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <strong style={{ color: '#14532d' }}>{edu.degree}</strong>
