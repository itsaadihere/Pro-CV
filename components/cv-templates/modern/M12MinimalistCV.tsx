    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          ...
          <div style={{ height: s(0.5), backgroundColor: '#9ca3af', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function M12MinimalistCV({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20) }}>
        <h1 style={{ fontSize: s(36), fontWeight: 900, color: '#000000', margin: 0, letterSpacing: '-1px' }}>{data.fullName}</h1>
        <div style={{ height: s(1), backgroundColor: '#9ca3af', margin: `${s(8)} 0` }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(10), color: '#6b7280' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#9ca3af', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
