...
import { CVTemplateProps } from '@/types/cv'
    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(16), borderLeft: `${s(4)} solid #000000`, paddingLeft: s(8) }}>
...
export default function M13ModernMinimalist1({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: s(20), borderBottom: `${s(1)} solid #e5e7eb`, paddingBottom: s(12) }}>
        <div>
          <h1 style={{ fontSize: s(26), fontWeight: 800, color: '#000000', margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(12), color: '#4b5563', marginTop: s(4), fontWeight: 500 }}>{data.jobTitle}</div>
        </div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#00000015', borderRadius: s(4), color: '#000000', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #000000` }}>
            {skill}
          </span>
        ))}
      </div>
