    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#1f2937', textTransform: 'uppercase' }}>{label}</h2>
...
export default function M14ModernMinimalist({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
