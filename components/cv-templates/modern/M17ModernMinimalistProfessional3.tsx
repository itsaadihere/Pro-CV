...
import { CVTemplateProps } from '@/types/cv'
    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(18), textAlign: 'center' }}>
          <div style={{ height: s(0.5), backgroundColor: '#b45309', margin: `${s(4)} auto` }} />
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#b45309', letterSpacing: '0.12em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(0.5), backgroundColor: '#b45309', margin: `${s(4)} auto` }} />
        </div>
      );
    };
...
export default function M17ModernMinimalistProfessional3({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: s(24) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 700, color: '#b45309', margin: 0, fontFamily: 'Georgia, serif' }}>{data.fullName}</h1>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: s(10), margin: `${s(6)} 0` }}>
          <div style={{ height: s(1), width: s(40), backgroundColor: '#b45309' }} />
          <span style={{ color: '#b45309', fontSize: s(12) }}>◆</span>
          <div style={{ height: s(1), width: s(40), backgroundColor: '#b45309' }} />
        </div>
        <div style={{ fontSize: s(11), fontStyle: 'italic', color: '#4b5563', marginBottom: s(4) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(12), fontSize: s(9.5), color: '#4b5563' }}>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <strong style={{ fontSize: s(11), color: '#b45309' }}>{job.title}</strong>
                <span style={{ fontSize: s(9), color: '#4b5563' }}>{job.startDate} – {job.endDate}</span>
              </div>
              <div style={{ fontSize: s(10), color: '#b45309', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <strong style={{ color: '#b45309' }}>{edu.degree}</strong>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ fontSize: s(9.5) }}>
            <strong style={{ color: isTwoColumn ? '#ffffff' : '#b45309' }}>{cat}: </strong>
