...
import { CVTemplateProps } from '@/types/cv'
    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(16), borderLeft: `${s(4)} solid #ea580c`, paddingLeft: s(8) }}>
          <h2 style={{ fontSize: s(12), fontWeight: 800, color: '#1f2937', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
        </div>
      );
    };
    

export default function M19ModernProfessional({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20) }}>
        <h1 style={{ fontSize: s(26), fontWeight: 850, color: '#1f2937', margin: 0 }}>{data.fullName}</h1>
        <div style={{ height: s(4), backgroundColor: '#ea580c', width: s(60), marginTop: s(6), borderRadius: s(2) }} />
        <div style={{ fontSize: s(12), color: '#4b5563', marginTop: s(8), display: 'flex', gap: s(16), flexWrap: 'wrap' }}>
          <strong style={{ color: '#1f2937' }}>{data.jobTitle}</strong>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#ea580c', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#ea580c15', borderRadius: s(4), color: '#1f2937', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #ea580c` }}>
            {skill}
          </span>
        ))}
      </div>
