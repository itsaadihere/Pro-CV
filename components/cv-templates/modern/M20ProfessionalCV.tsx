    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 700, color: '#d97706', letterSpacing: '0.08em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#d97706', marginTop: s(3), opacity: 0.4 }} />
        </div>
      );
    };
    

export default function M20ProfessionalCV({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: '#111827', color: '#ffffff', padding: `${s(22)} ${s(32)}`, textAlign: 'center', marginBottom: s(20), borderRadius: s(4) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 850, margin: 0, letterSpacing: '0.5px' }}>{data.fullName}</h1>
        <div style={{ fontSize: s(12), color: '#d97706', marginTop: s(4), fontWeight: 600 }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(16), fontSize: s(10), color: '#9ca3af', marginTop: s(8) }}>
        ...
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ marginBottom: s(2) }}>
            ...
            <div style={{ height: s(5), backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: s(2.5), overflow: 'hidden' }}>
              <div style={{ width: '85%', height: '100%', backgroundColor: '#d97706' }} />
            </div>
