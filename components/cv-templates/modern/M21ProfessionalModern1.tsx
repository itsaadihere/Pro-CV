    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: isSidebar ? '#1d4ed8' : '#1d4ed8', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
...
export default function M21ProfessionalModern1({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: s(12), opacity: 0.9, marginTop: s(2) }}>{data.jobTitle}</div>
        </div>
    ...
    <div style={{ marginBottom: s(16) }}>
      <h3 style={{ fontSize: s(10), fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isLeftSidebar ? '#ffffff' : '#1d4ed8', marginBottom: s(6) }}>Contact</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(8) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ marginBottom: s(2) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: s(9.5), fontWeight: 600, color: isTwoColumn ? '#ffffff' : '#1d4ed8', marginBottom: s(2) }}>
              <span>{cat}</span>
            </div>
            <div style={{ height: s(5), backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: s(2.5), overflow: 'hidden' }}>
              <div style={{ width: '85%', height: '100%', backgroundColor: '#1d4ed8' }} />
            </div>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <strong style={{ color: isLeftSidebar ? '#ffffff' : '#1d4ed8' }}>{edu.degree}</strong>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <strong style={{ fontSize: s(11), color: '#1d4ed8' }}>{job.title}</strong>
                <span style={{ fontSize: s(9), color: '#4b5563' }}>{job.startDate} – {job.endDate}</span>
              </div>
              <div style={{ fontSize: s(10), color: '#1d4ed8', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <strong style={{ color: '#1d4ed8' }}>{edu.degree}</strong>
