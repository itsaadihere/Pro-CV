    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#10b981', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#10b981', marginTop: s(3), width: '100%', borderRadius: s(1) }} />
        </div>
      );
    };
    

export default function M22ProfessionalModern2({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), borderLeft: `${s(6)} solid #10b981`, paddingLeft: s(16) }}>
        <h1 style={{ fontSize: s(28), fontWeight: 850, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(12), color: '#10b981', fontWeight: 600, marginTop: s(4) }}>{data.jobTitle}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#10b981', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#10b98115', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #10b981` }}>
            {skill}
          </span>
        ))}
      </div>
