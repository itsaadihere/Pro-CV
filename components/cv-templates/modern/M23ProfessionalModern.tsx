...
export default function M23ProfessionalModern({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: '#2563eb', color: '#ffffff', padding: `${s(20)} ${s(24)}`, marginBottom: s(20), borderRadius: s(4) }}>
        <h1 style={{ fontSize: s(22), fontWeight: 800, margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(11), opacity: 0.9, marginTop: s(4) }}>{data.jobTitle}</div>
      </div>
    ...
    <div style={{ marginBottom: s(16) }}>
      <h3 style={{ fontSize: s(10), fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isLeftSidebar ? '#ffffff' : '#1e3a8a', marginBottom: s(6) }}>Contact</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ fontSize: s(9.5) }}>
            <strong style={{ color: isTwoColumn ? '#ffffff' : '#1e3a8a' }}>{cat}: </strong>
      ...
      <div style={{ marginBottom: s(16) }}>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <strong style={{ color: isLeftSidebar ? '#ffffff' : '#1e3a8a' }}>{edu.degree}</strong>
          ...
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <strong style={{ fontSize: s(11), color: '#1e3a8a' }}>{job.title}</strong>
        ...
        {data.education.map((edu, i) => (
          <div key={i} style={{ marginBottom: s(8), fontSize: s(9.5) }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <strong style={{ color: '#1e3a8a' }}>{edu.degree}</strong>
