...
import { CVTemplateProps } from '@/types/cv'
    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(12), marginTop: s(18) }}>
          <h2 style={{ fontSize: s(11), fontWeight: 600, color: '#000000', letterSpacing: '0.12em', margin: 0, textTransform: 'uppercase', fontFamily: 'Georgia, serif' }}>{label}</h2>
          <div style={{ height: s(1), backgroundColor: '#000000', marginTop: s(4) }} />
        </div>
      );
    };
    

export default function M25WhiteBlackElegantCorporate({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: s(24) }}>
        <h1 style={{ fontSize: s(26), fontWeight: 700, color: '#000000', margin: 0, fontFamily: 'Georgia, serif' }}>{data.fullName}</h1>
        <div style={{ height: s(1), backgroundColor: '#000000', margin: s(6) + ' 0', borderTop: `${s(1)} solid #000000`, borderBottom: `${s(1)} solid #000000`, padding: '1px 0' }} />
        <div style={{ fontSize: s(12), fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#000000', marginBottom: s(6) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(16), fontSize: s(10), color: '#4b5563' }}>
