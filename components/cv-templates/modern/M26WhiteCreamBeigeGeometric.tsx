    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(10), fontWeight: 700, color: '#4b5563', letterSpacing: '0.15em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(0.5), backgroundColor: '#d97706', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function M26WhiteCreamBeigeGeometric({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: s(24), position: 'relative' }}>
        <div>
          <h1 style={{ fontSize: s(26), fontWeight: 800, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(12), color: '#d97706', fontWeight: 600, marginTop: s(4) }}>{data.jobTitle}</div>
          <div style={{ display: 'flex', gap: s(12), fontSize: s(10), color: '#4b5563', marginTop: s(6) }}>
            {data.email && <span>{data.email}</span>}
            {data.phone && <span>{data.phone}</span>}
            {data.location && <span>{data.location}</span>}
          </div>
        </div>
        <div style={{ width: s(40), height: s(40), borderRadius: '50%', backgroundColor: '#d97706', opacity: 0.2 }} />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#d97706', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#d9770615', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #d97706` }}>
            {skill}
          </span>
        ))}
      </div>
