    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#06b6d4', letterSpacing: '0.05em', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#06b6d4', marginTop: s(3), width: '100%', borderRadius: s(1) }} />
        </div>
      );
    };
    

export default function M28WhiteSimpleStudent({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: s(20) }}>
        <h1 style={{ fontSize: s(24), fontWeight: 800, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
        <div style={{ fontSize: s(11), color: '#06b6d4', fontWeight: 600, marginTop: s(2) }}>{data.jobTitle}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: s(16), fontSize: s(10), color: '#4b5563', marginTop: s(6) }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#06b6d4', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 12).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(6)}`, backgroundColor: '#f3f4f6', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, textAlign: 'center' }}>
            {skill}
          </span>
        ))}
      </div>
