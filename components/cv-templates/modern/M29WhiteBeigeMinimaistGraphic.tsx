    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(12), fontWeight: 700, color: '#1e293b', fontFamily: 'Georgia, serif', margin: 0 }}>{label}</h2>
          <div style={{ height: s(1.5), backgroundColor: '#d6b896', marginTop: s(4) }} />
        </div>
      );
    };
    

export default function M29WhiteBeigeMinimaistGraphic({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(24), borderBottom: `${s(1)} solid #d6b896`, paddingBottom: s(16) }}>
        <h1 style={{ fontSize: s(20), fontWeight: 700, color: '#1e293b', margin: 0, fontFamily: 'Georgia, serif' }}>{data.fullName}</h1>
        <div style={{ fontSize: s(10.5), color: '#4b5563', marginTop: s(4), fontStyle: 'italic' }}>{data.jobTitle}</div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: s(6) }}>
        {Object.entries(data.technicalSkills).map(([cat, skills], i) => (
          <div key={i} style={{ fontSize: s(9.5) }}>
            <strong style={{ color: isTwoColumn ? '#ffffff' : '#1e293b' }}>{cat}: </strong>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#d6b896', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
