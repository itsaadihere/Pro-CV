    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(12), fontWeight: 800, color: '#16a34a', margin: 0, textTransform: 'uppercase' }}>{label}</h2>
          <div style={{ height: s(1), backgroundColor: '#16a34a', marginTop: s(3) }} />
        </div>
      );
    };
    

export default function M30WhiteGreenSimpleStudent({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
