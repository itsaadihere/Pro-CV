    ...
    const SectionHeader = ({ label, scale }: { label: string; scale: number }) => {
      ...
      return (
        <div style={{ marginBottom: s(10), marginTop: s(14) }}>
          <h2 style={{ fontSize: s(11.5), fontWeight: 700, color: '#1e293b', textTransform: 'uppercase' }}>{label}</h2>
...
export default function M31WhiteSimpleStudent3({ data, scale = 1 }: CVTemplateProps) {
  const s = (n: number) => `${n * scale}px`
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: s(20), display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: s(22), fontWeight: 800, color: '#1e293b', margin: 0 }}>{data.fullName}</h1>
          <div style={{ fontSize: s(11), color: '#a78bfa', fontWeight: 600, marginTop: s(2) }}>{data.jobTitle}</div>
        </div>
        <div style={{ fontSize: s(10), color: '#4b5563', textAlign: 'right', display: 'flex', flexDirection: 'column', gap: s(2) }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: s(12) }}>
          {data.experience.map((job, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <div style={{ fontSize: s(10), color: '#a78bfa', fontWeight: 600, marginBottom: s(4) }}>{job.company}</div>
    ...
    {!isTwoColumn && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: s(6) }}>
        {Object.values(data.technicalSkills).flat().slice(0, 15).map((skill, i) => (
          <span key={i} style={{ padding: `${s(3)} ${s(8)}`, backgroundColor: '#a78bfa15', borderRadius: s(4), color: '#1e293b', fontSize: s(9.5), fontWeight: 500, borderLeft: `${s(2)} solid #a78bfa` }}>
            {skill}
          </span>
        ))}
      </div>
