...
function analyzePossibleSolution(message: string, stack?: string): string {
  ...
  if (errLower.includes('puppeteer') || errLower.includes('pdf')) {
    return 'PDF Puppeteer generation error. Check server memory limit or headless Chrome browser dependencies.'
  }
